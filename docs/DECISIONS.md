...
**Switching sites** therefore means re-running consent, which is where Atlassian's own picker lives: **sign out, sign in, choose a different site**. Because `/oauth/start` sends `prompt=consent`, the chooser reliably reappears. Re-consenting with the same Atlassian identity updates the existing account row in place — same account, same API keys, new site — which is asserted by a test.

There is deliberately no "Switch site" button. A first attempt added one, but all it could do was sign you out and restart the flow — a second, differently-labelled Sign out. Two buttons for one action, one of which hid the fact that it ends your session, is worse than a line of text saying what to do. The site card states it instead.
...
## 6. *(withdrawn)* Passwords: scrypt from `node:crypto`

Deliberately left numbered and empty so the references from other docs stay stable. This ADR chose a password-hashing strategy; #2 removed passwords from the product entirely, which retired it.

## 7. Jira tokens encrypted at rest: AES-256-GCM

**Decision:** Access + refresh tokens encrypted with AES-256-GCM (fresh IV per encryption, authentication tag detects tampering); key is a 32-byte env secret generated by `npm run setup`. Tokens are never returned to the client and never logged. They exist in exactly one place — the `accounts` row — because the site is granted during the OAuth exchange itself (#2c): there is no intermediate "picking a site" step, so no token ever has to be parked in the session.
...
**Implementation note:** `POST /rest/api/3/search/jql` — the older `/rest/api/3/search` now returns 410 Gone, and the replacement returns no issue fields unless `fields` is listed explicitly.

The project key *is* interpolated into the JQL string, and what makes that safe is `projectKeySchema` in `shared/`, which constrains it to `/^[A-Za-z][A-Za-z0-9_]*$/` — no quotes, spaces or operators can survive parsing. That regex is therefore load-bearing security rather than input tidiness, so **all three routes that can reach JQL parse through the shared schema**, not through a local "short string" copy: `POST`/`GET /api/v1/findings` and `GET /api/tickets/recent`. Injection attempts (`SEC ORDER BY created ASC`, `X OR labels=identityhub`, …) are rejected as `VALIDATION_ERROR` before any Jira call, which is asserted by tests.
...
The general rule this follows: if a missing value makes the process useless, refuse to start. Don't ship a half-working mode whose only job is to apologise. The digest's variables (`GROQ_API_KEY`, `DIGEST_USER_EMAIL`, `DIGEST_PROJECT_KEY`) are optional *to the server*, which never imports the digest, and required by the digest script itself — so the check lives there rather than in the boot schema, and `npm run dev` doesn't demand a Groq key from someone who only wants the app.

## 11. Error contract: one envelope, human messages, stable codes

**Decision:** Every non-2xx response is `{ error: { code, message, details? } }`. Codes are stable and machine-readable (`JIRA_RECONNECT_REQUIRED`, `API_KEY_INVALID`, `VALIDATION_ERROR` with per-field details); messages are written for humans and say *what to do next* ("Your Jira authorization expired or was revoked. Please sign in again", "Check that the client ID/secret and callback URL match your Atlassian app exactly"). Unexpected errors log the stack server-side and return a generic message — internals never leak. Public-API auth runs before routing, so unauthenticated callers cannot enumerate routes.
...
- **`GROQ_API_KEY` is required, checked before the run starts** — Groq summarizes (model configurable via `DIGEST_MODEL`, default `llama-3.3-70b-versatile`). An earlier revision treated the key as optional and filed a raw excerpt without one; that made "the summary" silently mean two different things, and a ticket already in Jira is the worst place to discover which one you got. The digest now stops with a message naming the missing variable (#10b's rule, applied to the script instead of the server). The extractive summary survives only as the fallback for a Groq call that fails *mid-run*, after the post is already fetched — covered by tests that stub a failing fetch.
...
**Decision:** `@hookform/resolvers` currently ships a self-conflicting optional-peer graph (valibot ranges) that fails a clean `npm install`. Rather than forcing `--legacy-peer-deps` project-wide, the Zod→react-hook-form resolver is ~25 in-house lines, and the shadcn `form` component (whose registry entry pulls that package) was added by hand. `npm install` is clean and every line of both pieces is explainable.

*(This removed the resolver dependency; it is not a claim that the whole tree audits clean — the `react-router` pin in #15 currently does not.)*
...
| `pino` + `pino-pretty`, pretty transport in every mode | Structured logs for two dependencies; every call site logs ids and messages only, never tokens | Drop the pretty transport outside development (ship raw NDJSON), add `redact` paths and request ids as belt-and-braces |
...
| `react-router` pinned to `~7.11.0` | **Known-stale pin: this does not audit clean.** `npm audit` reports one high-severity finding against it (see the note below the table) | Bump to ≥ 7.18.0, which is the first release outside every advisory range that currently covers 7.11 |
...
### Outstanding: the `react-router` pin is stale

This pin was originally taken to *avoid* an advisory, on the reasoning that 7.11 predated the vulnerable RSC code. That reasoning no longer holds and the row above has been corrected: `npm audit` currently reports **1 high-severity vulnerability**, and 7.11.0 sits inside the affected range of several advisories rather than ahead of them — including [GHSA-h5cw-625j-3rxh](https://github.com/advisories/GHSA-h5cw-625j-3rxh) (CSRF, `>=7.0.0 <=7.11.0`, i.e. 7.11 is affected) and [GHSA-2w69-qvjg-hvjx](https://github.com/advisories/GHSA-2w69-qvjg-hvjx) (XSS via open redirect, `<=7.11.0`).

Most of those advisories describe SSR/RSC surfaces this client-only SPA does not run. One does apply to code paths this app uses: [GHSA-wrjc-x8rr-h8h6](https://github.com/advisories/GHSA-wrjc-x8rr-h8h6) — open redirect via a backslash in `<Link>` / `useNavigate`, fixed in 7.18.0. The app only ever navigates to fixed internal paths, so there is no attacker-controlled redirect target today, but the pin is a liability rather than a mitigation and the honest status is "needs bumping", not "audits clean".
